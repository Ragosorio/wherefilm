...
public enum MobileCLIPVariant: String, Sendable, CaseIterable {
    ...
    /// a property of how the model was trained. Its range is roughly half the
    /// spread of the original OpenAI CLIP, and swapping models has to swap these
    /// numbers too, or "how confident is this?" becomes meaningless. The UI can
    /// offer stricter or broader exploration by overriding it per search.
    ///
    /// Re-measured 2026-09-05 with `wherefilm calibrate`, which scores every
    /// preview in a library against a labelled set and reports where the right
    /// and wrong answers actually separate. Two findings changed this number and
    /// one changed what to expect of it:
    ///
    ///  - The best single threshold on the real-media library is ~0.22 by F1, and
    ///    0.18 is where the trade turns: nonsense queries that returned something
    ///    fell from 38% to 12%, for one point of Recall@10 and 0.04 of nDCG.
    ///    Halving false positives is worth that; 0.22 costs more and buys nothing
    ///    further.
    ///  - S0 and S2 separate almost identically — relevant/irrelevant medians
    ///    0.191/0.133 against 0.177/0.122 — so the bigger model is not better at
    ///    *this*, which is why reranking with it does not pay.
    ///  - The distributions overlap heavily either way. No single cosine
    ///    threshold makes this model reliable on its own, which is the case for
    ///    fusing four channels rather than trusting one.
    public var similarityFloor: Float {
        switch self {
        case .s0, .s1, .s2, .blt: 0.18
        }
    }
